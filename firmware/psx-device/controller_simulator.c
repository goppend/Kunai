...
volatile PSXOutputState *outputState;
...
void processRumble(uint8_t index, uint8_t value) {
	...
	switch(motorBytes[index - 1]) {
		case 0x00: {
			outputState->small_motor = value;
			break;
		}
		case 0x01: {
			outputState->large_motor = value;
			break;
		}
	}
}
...
void processPollConfigStatus() {
	...
	uint8_t buf[7] = {
		0x5A,
		(mode == MODE_DIGITAL) ? 0x00 : 0xFF,
		(mode == MODE_DIGITAL) ? 0x00 : 0xFF,
		(mode == MODE_DIGITAL) ? 0x00 : 0x03,
		(mode == MODE_DIGITAL) ? 0x00 : 0x00,
		0x00,
...
void processPoll() {
	...
	switch(mode) {
		case MODE_DIGITAL: {
			uint8_t buf[3] = {
				0x5A,
				inputState->buttons&0xFF,
				(inputState->buttons>>8)&0xFF
			};
		...
		case MODE_ANALOG_PRESSURE: {
			uint8_t buf[19] = {
				...
				inputState->ljoy_x,
				inputState->ljoy_y,
				(mode == MODE_DIGITAL) ? (inputState->buttons & 1<<RIGHT ? 0x00 : 0xFF) : inputState->dpad_rt,
				(mode == MODE_DIGITAL) ? (inputState->buttons & 1<<LEFT  ? 0x00 : 0xFF) : inputState->dpad_lt,
				(mode == MODE_DIGITAL) ? (inputState->buttons & 1<<UP    ? 0x00 : 0xFF) : inputState->dpad_up,
				(mode == MODE_DIGITAL) ? (inputState->buttons & 1<<DOWN  ? 0x00 : 0xFF) : inputState->dpad_dn,
				(mode == MODE_DIGITAL) ? (inputState->buttons & 1<<TRI   ? 0x00 : 0xFF) : inputState->face_tri,
				(mode == MODE_DIGITAL) ? (inputState->buttons & 1<<CIR   ? 0x00 : 0xFF) : inputState->face_cir,
				(mode == MODE_DIGITAL) ? (inputState->buttons & 1<<CRS   ? 0x00 : 0xFF) : inputState->face_crs,
				(mode == MODE_DIGITAL) ? (inputState->buttons & 1<<SQU   ? 0x00 : 0xFF) : inputState->face_squ,
				(mode == MODE_DIGITAL) ? (inputState->buttons & 1<<L1    ? 0x00 : 0xFF) : inputState->shld_l1,
				(mode == MODE_DIGITAL) ? (inputState->buttons & 1<<R1    ? 0x00 : 0xFF) : inputState->shld_r1,
				(mode == MODE_DIGITAL) ? (inputState->buttons & 1<<L2    ? 0x00 : 0xFF) : inputState->shld_l2,
				(mode == MODE_DIGITAL) ? (inputState->buttons & 1<<R2    ? 0x00 : 0xFF) : inputState->shld_r2 
			};
	...
	for(uint8_t i = 0; i < 7; i++) {
		switch(i) {
			...
			case 1: {
				...
				outputState->analog_led = mode != MODE_DIGITAL;
				break;
			}
			case 2: {
				SEND(0x00);
...
// main entrypoint for talking to a Playstation or Playstation 2 console via the controller port
...
// Initializes i/o for talking to a Playstation or Playstation 2 console
// uint pio
// PSXInputState* indata
// PSXOutputStat* outdata
// void* reset_pio
